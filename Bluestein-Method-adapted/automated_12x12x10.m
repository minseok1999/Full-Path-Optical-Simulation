...
tic;
%phase modulation%
matFileName = sprintf('L%dqutrit_projection.csv',p);
phi2=fliplr(readmatrix(fullfile('C:\Users\rmins\Desktop\SynologyDrive\01 개인폴더 (shared)\류민석\SLM\qutrit phase projection data for Minseok 1920',matFileName)));
dim2=size(phi2);
mx4=dim2(1);
my4=dim2(2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=1:10
 ...
 saveas(fig(k), fullfile(sprintf('C:/Users/rmins/Desktop/SynologyDrive/12 Experimental data/SLM/Simulation/Intensity masking/image/input%d/proj%d',q,p),sprintf('masking_%dto%d_%d',q,p,k)), 'jpeg')
  
  %%%%%csv making
writematrix(g8,fullfile(sprintf('C:/Users/rmins/Desktop/SynologyDrive/12 Experimental data/SLM/Simulation/Intensity masking/csv/input%d/proj%d',q,p),sprintf('masking_%dto%d_%d.csv',q,p,k)));
