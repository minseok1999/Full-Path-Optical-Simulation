...
pixel0=8;
%phase modulation%
phi=readmatrix('002L2qutritIntensity.csv');
dim=size(phi);
mx0=dim(1);
my0=dim(2);
...
w=ff.*lamda./pi./waist;
%%%%%
gdot=exp(-(x8.^2+y8.^2)./(1.5e3)^2);
overlap=(sum(abs(g8.*conj(gdot)),'all')^2)./(sum(abs(g8).^2,'all'))./(sum(abs(gdot).^2,'all'))

%%%%saving images

for k=1:10
  
  saveas(fig(k), fullfile('C:\Users\rmins\Desktop\matfig',sprintf('masking_2to%d_%d',p,k)), 'jpeg')
