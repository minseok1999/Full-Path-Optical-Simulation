...
%%%%beam waist before tight focusing with 8mm lens is 0.8mm

gdot=exp(-(x8.^2+y8.^2)./(0.8e3)^2);
overlap(q,p)=(abs(sum(g8.*conj(gdot),'all'))^2)./(sum(abs(g8).^2,'all'))./(sum(abs(gdot).^2,'all'));
...
for k=1:12
  tic;
  saveas(fig(k), fullfile(sprintf('C:/Users/YQOG_LAB/SynologyDrive/12 Experimental data/SLM/Simulation/Intensity masking/image/input%d/proj%d',q,p),sprintf('masking_%dto%d_%d',q,p,k)), 'jpeg')
 toc
  %%%%%csv making
tic;
writematrix(g8,fullfile(sprintf('C:/Users/YQOG_LAB/SynologyDrive/12 Experimental data/SLM/Simulation/Intensity masking/csv/input%d/proj%d',q,p),sprintf('masking_%dto%d_%d.csv',q,p,k)));
toc  
...
writematrix(overlap,fullfile('//Yqog_nas/yqog/12 Experimental data/SLM/Simulation/Intensity masking','overlap.csv'));
end